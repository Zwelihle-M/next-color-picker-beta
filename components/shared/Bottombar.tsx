        ...
        {sidebarNavigationlinks.map((link) => {
          ...
          return (
            <Link
              href={link.route}
              key={link.label}
              className={`relative flex flex-col items-center gap-2 rounded-lg p-2 sm:flex-1 sm:px-2 sm:py-2.5${isActive && "bg-razzmatazz"}`}
            >
              {/* <Image
                src={link.imgURL}
                alt={link.label}
              ...
              /> */}
              <p className='object-contain'>{link.imgURL}</p>
